...
class Library:
    ...
    def search_by_composition(only=[],not_including=[],any_of=[]):
        elt_url = 'https://app-test.hpc.nrel.gov:8095/api/sample_library?element='
        for i in only:
            elt_url = elt_url+str(i)+','
        response = urllib2.urlopen(elt_url, context=ctx)
    ...
    def properties(self):
        url = 'https://app-test.hpc.nrel.gov:8095/api/sample_library/'+str(self.identity)
    ...
    def spectra(self,which):
        ...
        df = pd.DataFrame()
        for k in positions:
            url = 'https://app-test.hpc.nrel.gov:8095/api/sample/'+str(k)
